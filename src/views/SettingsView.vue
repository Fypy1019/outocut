...
const secrets = reactive({ bailian_api_key: '', minimax_api_key: '', deepseek_api_key: '', douyin_cookie: '' })
    ...
    <section class="panel settings-section-card">
      ...
      <el-form class="copy-config-form" label-width="100px">
        ...
        <el-form-item label="百炼 Key">
          <el-input v-model="secrets.bailian_api_key" type="password" show-password :disabled="settings.rewrite_provider === 'deepseek' && settings.asr_provider !== 'bailian'" :placeholder="settings.configured.bailian ? '已安全配置，留空表示不修改' : '输入阿里云百炼 API Key'" />
          ...
          <span class="field-help">用于文案改写（选择百炼时）和抖音链接口播语音转写（选择百炼 API 时）；只有两项都不使用百炼时才禁用。</span>
        </el-form-item>
